...
class MiniSom(object):
    ...
    def pca_weights_init(self, data):
        ...
        for i, c1 in enumerate(linspace(-1, 1, len(self._neigx))):
            for j, c2 in enumerate(linspace(-1, 1, len(self._neigy))):
                self._weights[i,
                              j] = c1 * pc[pc_order[0]] + c2 * pc[pc_order[1]]
        ...
        while not it.finished:
            for ii in range(it.multi_index[0] - 1, it.multi_index[0] + 2):
                for jj in range(it.multi_index[1] - 1, it.multi_index[1] + 2):
                    if (ii >= 0 and ii < self._weights.shape[0] and jj >= 0 and
                            jj < self._weights.shape[1]):
    ...
    def topographic_error(self, data):
        ...
        for x in data:
            ...
            bmu_1 = unravel_index(
                where(indexes == 0)[0][0], self._activation_map.shape)
            bmu_2 = unravel_index(
                where(indexes == 1)[0][0], self._activation_map.shape)
